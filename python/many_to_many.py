...
def func(): 
    ...
    hServer2 = liner.Client("server2", "topic_server2", "localhost:2258", "redis://127.0.0.1/")
    ...
    for i in range(SEND_CYCLE_COUNT):
        ...
        for j in range(MESS_SEND_COUNT):
            if not hServer1.send_all("topic_client", b, len(b), True):
                raise Exception('error send_all')
        for j in range(MESS_SEND_COUNT):
            if not hServer2.send_all("topic_client", b, len(b), True):
                raise Exception('error send_all')  
        send_end = time.time()

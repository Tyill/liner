...
fn  main() {
    ...
    let mut server2 = Liner::new("server2", "topic_server2", "localhost:2259", "redis://localhost/");
    ...
    server2.run(Box::new(move |_to: &str, _from: &str,  _data: &[u8]|{}));
    ...
    for _ in 0..SEND_CYCLE_COUNT{
        ...
        for _ in 0..MESS_SEND_COUNT{
            ...
            server2.send_all("topic_client", array.as_slice(), true);
        }

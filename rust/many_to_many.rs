...
fn  main() {
    unsafe {
        ...
        let server2 = CString::new("server2").unwrap();
        ...
        let localhost5 = CString::new("localhost:2259").unwrap();
        ...
        let topic_server2 = CString::new("topic_server2").unwrap();
        let mut hserver2 = liner_broker::ln_new_client(server2.as_ptr(), 
                                                                topic_server2.as_ptr(),
                                                                localhost5.as_ptr(),
                                                                dbpath.as_ptr());
    
        liner_broker::ln_clear_stored_messages(&mut hserver1);
        liner_broker::ln_clear_stored_messages(&mut hserver2);
        ...
        liner_broker::ln_clear_addresses_of_topic(&mut hserver2);
        ...
        liner_broker::ln_run(&mut hserver2, cb_server, std::ptr::null_mut());
    ...
    for _ in 0..SEND_CYCLE_COUNT{
        ...
        for _ in 0..MESS_SEND_COUNT{
            ...
            liner_broker::ln_send_all(&mut hserver2, topic_client.as_ptr(), array.as_ptr(), array.len(), true);
        }

...
from _support import ensure_release_lib, free_port, temp_shared_db  # noqa: E402


def spawn_client1(db_path: str, client_addr: str, *extra_args: str):
...
def main() -> int:
    ...
    try:
        ...
        h2 = liner.Client.new_sqlite("client2", "topic2", addr2, db_path, "")
        ...
        c1 = spawn_client1(db_path, addr1, "--subscr-topic=topic_sub")
        try:
            time.sleep(2.5)
            got.clear()
            ...
            try:
                ...
                try:
                    ...
                    pass
            unsub_proc = spawn_client1(db_path, addr1, "--unsubscr-topic=topic_sub")
            try:
                time.sleep(1.0)
            finally:
                try:
        ...
        c1 = spawn_client1(db_path, addr1, "--unsubscr-topic=topic_sub")
        try:
            time.sleep(2.5)
            h2.refresh_address_topic("topic_sub")
            got.clear()

...
| `unsubscribed` | после **`unsubscribe`** в running-состоянии | обновление кэша для этого топика |
...
**`refresh_address_topic`** — явное принудительное обновление кэша; в обычном сценарии connect/subscribe в рантайме он не обязателен.

...
| **`localhost` (строка bind)** | **TCP listen-адрес**, к которому этот процесс привязывается (например `127.0.0.1:2255` или `0.0.0.0:0`). Хранится на клиенте как строка bind; advertise её не перезаписывает. |
| **Опубликованный / advertise-адрес** | Строка, записываемая в каталог топиков для подключения пиров. По умолчанию — bound listen-адрес после `run`; опциональный `set_advertise_addr` переопределяет его (порт `0` подставляется из реального bind). Доступен как `published_addr`, пока клиент зарегистрирован. |
| **Топик (строка)** | Логическое имя, которым пользуются пиры (`send_to("other_topic", …)`). |
| **Адрес listener’а** | **Достижимый TCP endpoint** другого клиента, берётся из хранилища при отправке (поле опубликованного каталога). |

## Как работает маршрутизация (высокий уровень)

1. **Каталог: топик → TCP-адреса**  
   Когда клиент регистрирует топик (`regist_topic`), хранилище фиксирует отображение: **имя топика → (опубликованный адрес → `unique_name` этого клиента)**. Опубликованный адрес — строка advertise, если задана, иначе bound listen-адрес.  
   ...
   Операторы также могут вызвать **`list_addresses(topic)`** / `get_topic_directory`, чтобы прочитать пары `(addr, unique_name)` без отправки.
...
| Офлайн-очередь застряла | `LLEN lnr_connection:{id}:messages` (или API `pending_count` для этого sender’а) | `SELECT COUNT(*) FROM conn_messages WHERE connection_key = ?;` (или API `pending_count`) |
| Дедуп / курсор ack | `GET lnr_connection:{id}:mess_number` | `SELECT v FROM conn_mess_number WHERE connection_key = ?;` |
| Неверный пир / старый порт | Проверьте имена полей в `…:addr` на актуальные **опубликованные** адреса (`published_addr` / advertise) | То же в **`topic_addr.addr`** |

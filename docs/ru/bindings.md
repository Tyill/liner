...
- **Поставляемый `python/liner.py`:** **`new_redis`**, **`new_sqlite`** (пять строк, **`receivers_json`**), **`new_postgres`** (четыре строки, сборка с **`--features postgres`**). См. [using-sqlite.md](using-sqlite.md) и [using-postgres.md](using-postgres.md).

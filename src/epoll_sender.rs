    ...
    for addr in addrs.lock().unwrap().clone(){
        ...
        match TcpStream::connect(&addr){
            Ok(stream)=>{
                ...
                streams.insert(stream.as_raw_fd(), Arc::new(Mutex::new(stream)));
            },
            Err(err)=>{
...
fn write_stream(stream_fd: RawFd,
    ...
    if let Some(stream) = streams.get(&stream_fd){
        let addr_to = stream.lock().unwrap().peer_addr().unwrap().to_string();
        if !&messages_new.lock().unwrap()[&addr_to].is_empty(){
            ...
            rayon::spawn(move || {
                ...
                let mut writer = BufWriter::new(stream.deref()); 
